...
-- Thêm cột LotSelection
ALTER TABLE ProductUnitConversion ADD LotSelection NVARCHAR(20) NULL;

-- Thêm cột SpecificLotId  
ALTER TABLE ProductUnitConversion ADD SpecificLotId INT NULL;
...
CREATE TABLE Inventory (
    ...
    CostPrice DECIMAL(18,2) NULL,      -- Giá nhập kho
    PackSize INT NULL,                 -- Số đơn vị trong 1 lốc (chỉ dùng cho PACK)
    LastUpdated DATETIME DEFAULT GETDATE(), -- Thời gian cập nhật cuối
    LotNumber NVARCHAR(50) NULL,       -- Số lô hàng
    LotDate DATETIME NOT NULL DEFAULT GETDATE(), -- Ngày tạo lô
    ExpiryDate DATE NULL,              -- Hạn sử dụng
    RemainingQuantity DECIMAL(18,2) NULL, -- Số lượng còn lại (cho tracking)
...
CREATE TABLE StockInDetail (
    ...
    InventoryID INT NULL,               -- NULL khi tạo, được set sau khi approve
    ProductID INT NOT NULL,             -- ID sản phẩm
    PackageType NVARCHAR(10) NOT NULL, -- Loại đóng gói
    Quantity DECIMAL(18,2) NOT NULL,   -- Số lượng
    UnitPrice DECIMAL(18,2) NOT NULL,  -- Giá nhập
    LotNumber NVARCHAR(50) NULL,       -- Số lô hàng
    ManufactureDate DATE NULL,          -- Ngày sản xuất
    ExpiryDate DATE NULL,              -- Hạn sử dụng
    
    CONSTRAINT FK_StockInDetail_StockIn FOREIGN KEY (StockInID) REFERENCES StockIn(StockInID) ON DELETE CASCADE,
    CONSTRAINT FK_StockInDetail_Product FOREIGN KEY (ProductID) REFERENCES Product(ProductID),
    CONSTRAINT CHK_StockInDetail_PackageType CHECK (PackageType IN ('BOX', 'UNIT', 'PACK', 'KG'))
);
