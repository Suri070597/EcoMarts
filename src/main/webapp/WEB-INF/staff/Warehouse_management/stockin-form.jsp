                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        ...
                        <div id="stockInForm" style="display:none;">
                            <form method="POST" action="${pageContext.request.contextPath}/staff/stockin"
                                  class="needs-validation" novalidate>
                                <input type="hidden" name="action" value="create">

                                <!-- Nhà cung cấp -->
                                <div class="mb-3">
                                    <label class="form-label" for="manufacturerId">Nhà cung cấp</label>
                                    <select class="form-control" id="manufacturerId" name="manufacturerId"
                                            required>
                                        <option value="">-- Chọn nhà cung cấp --</option>
                                        <c:forEach var="s" items="${manufacturers}">
                                            <option value="${s.manufacturerID}" <c:if
                                                        test="${param.manufacturerId == s.manufacturerID}">selected
                                                    </c:if>>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-control" id="receiverId" name="receiverId" required>
                                        ...
                                        <c:forEach var="r" items="${receivers}">
                                            <option value="${r.accountID}" ${param.receiverId eq r.accountID
                                                             ? 'selected' : '' }>
                                                        ${r.fullName}
                                                    </option>
                                            </c:forEach>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="date" class="form-control" id="date" name="date"
                                               value="${param.date}" required min="<%= java.time.LocalDate.now()%>">
                                        ...
                                        <div class="row g-2 mb-2">
                                            <div class="col-md-12" style="margin-bottom: 8px;">
                                                <input type="search" id="productSearch" class="form-control"
                                                       placeholder="Tìm sản phẩm theo tên..." autocomplete="off">
                    ...
                    <div class="card">
                        <div class="card-header" style="border-bottom: none;">
                            ...
                            <form action="${pageContext.request.contextPath}/staff/stockin" method="get"
                                  class="d-flex gap-2">
                                <input type="hidden" name="action" value="searchInventory" />
                                <input type="text" name="keyword" placeholder="Tìm kiếm phiếu nhập..."
                                       value="${keyword != null ? keyword : ''}" class="form-control form-control-sm">
                                ...
                                <tbody>
                                    <c:forEach var="stock" items="${stockIns}">
                                        <tr data-bs-toggle="collapse" data-bs-target="#details${stock.stockInID}"
                                            ...
                                            <td>${stock.dateIn}</td>
                                            <td
                                                style="font-weight:bold; color:${stock.status == 'Pending' ? '#e6b800' : (stock.status == 'Completed' ? '#28a745' : '#dc3545')}">
                                                ${stock.status}
                                            </td>
                                        </tr>
                ...
                <script>
                    document.addEventListener("DOMContentLoaded", function () {
                        ...
                        searchInput.addEventListener("input", function () {
                            ...
                            fetch(`${pageContext.request.contextPath}/staff/stockin?action=search&keyword=\${encodeURIComponent(keyword)}`)
                                    .then(res => res.json())
                                    .then(data => {
                        ...
                        function addProductToTable(item) {
                            ...
                            if (item.boxUnitName) {
                                const name = item.boxUnitName.toLowerCase();

                                if (name.includes('box')) {
                                    packageType = 'BOX';
                                    packSize = item.unitPerBox || 0;
